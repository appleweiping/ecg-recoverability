...
intervals we attach to it, and a graded ST-safety study. On PTB-XL the map ranks target leads
on a continuum by normalized identifiability and expected mV ambiguity---from limb leads,
anterior precordial ST (V1--V4) is far from identifiable while lateral V5/V6 are much less
ambiguous (none exactly identifiable)---and the total ST-threshold error rate is empirically
similar across the evaluated reconstructors ($\TotWrongLo$--$\TotWrongHi\%$; an empirical
observation, not a certified bound) while the false-positive/false-negative split is a
