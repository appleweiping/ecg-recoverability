...
$\bm M_{s,S}=\bm U\bm\Sigma\bm V^{\top}$ at tolerance $\varrho$, where $\bm U,\bm V$ have
orthonormal columns, so $\bm M_{s,S}^{+}=\bm V\bm\Sigma^{-1}\bm U^{\top}$ and
$\bm P_{\mathrm{obs}}=\bm M_{s,S}^{+}\bm M_{s,S}=\bm V\bm V^{\top}$ is the orthogonal projector
onto the retained row space $\operatorname{span}(\bm V)$ (the numerical row space of
$\bm M_{s,S}$ at tolerance $\varrho$; the $\varrho$-sensitivity sweep in Sec.~\ref{sec:exp}
reports where this truncation matters). Below $\eta,\kappa$ are understood at this $\varrho$.
