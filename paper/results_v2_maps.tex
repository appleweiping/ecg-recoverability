...
shared between truth and reconstruction; median over beats; baseline fallback fired in
$\approx$9\% of records), and count ST-threshold events ($|$ST$|{\ge}0.1$\,mV; false positive =
crossing in reconstruction not truth, false negative = vice versa; Table~\ref{tab:safety}). On
$\SafetyN$ common records the \emph{total} wrong-event rate is empirically similar across
reconstructors ($\TotWrongLo$--$\TotWrongHi\%$) and is \emph{dominated by false negatives}
(missed true crossings: limb$\to$precordial recovery smooths ST toward the population), with the
false-positive/false-negative split a reconstructor choice (paired record-bootstrap $\Delta$
significant). We do not derive a minimax/Bayes bound, so we call the shared total an empirically
similar total error rate, \emph{not} a certified floor; we report ST-threshold events, not
diagnoses.
