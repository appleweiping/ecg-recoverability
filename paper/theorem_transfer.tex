...
is \emph{relative}. Empirically it is not merely satisfied but predictive. On PTB-XL$\to$Chapman
(matched sinus, limb-6 $\to$ V2), the realized drift is Lipschitz in the chordal subspace distance
with a small, \emph{stable} constant $\eta$-drift$/2\sin(\theta^\star/2)\in[\TransferLipLo,\TransferLipHi]$
across a sixfold range of angles: the QRS dipolar subspace is nearly shared
($\theta^\star{=}\TransferAngleQRS^\circ$, drift $\TransferDriftQRS$), whereas the ST subspace is
near-orthogonal ($\theta^\star{=}\TransferAngleST^\circ$, drift $\TransferDriftST$)---the certificate
transfers precisely to the degree the dipolar geometry is shared, and the measured angle says so in
advance (\texttt{experiments/transfer\_bound.py}). This is the cross-cohort analogue of the
finite-sample $\sin\Theta$ term in Thm.~\ref{thm:floor}(iii): both control how a fitted subspace
perturbs the verdict.
