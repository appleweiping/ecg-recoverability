...
for $w=1,2,4,6$ (mean over $\NegSeeds$ seeds), with SD $\pm\NegStdsix$ at $w{=}6$; the
seed-level 95\% $t$-interval of the mean is $[\NegCIlo,\NegCIhi]$ and \emph{includes zero}, so
the gap is not significant at the seed level. The previously reported large effect does not
reproduce.
