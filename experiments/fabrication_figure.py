"""Figure: the null-space dipolar energy ratio R_Q (exploratory descriptor).
...
results/fabrication_diffusion.json (GPU: DDPM vs guidance) and draws:
  (a) R_Q per observed-lead config for the abstaining reconstructors
...
def main():
    ...
    ax.set_ylabel(r"null-space energy ratio $R_Q$ (QRS)")
    ax.set_title("(a) Linear reconstructors: null-space energy $R_Q$ (exploratory)", fontsize=9.5)
    ...
    if diff:
        ...
        ax.set_ylabel(r"diffusion null-space energy ratio $R_Q$")
