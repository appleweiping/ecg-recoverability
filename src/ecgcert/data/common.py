...
def unit_scale_to_mv(unit: str | None) -> float:
    key = str(unit or "mV").strip().replace("μ", "u").replace("µ", "u").lower()
...
def canonicalize_wfdb_record(record) -> tuple[np.ndarray, dict]:
    ...
    units = tuple(str(value) for value in (getattr(record, "units", None) or ["mV"] * len(names)))
    ...
    return signal, {
        "input_leads": names,
        "input_units": tuple(units[index] for index in order),
